...
// ============================================================
// 🔑 YOUR WAQI TOKEN — paste it here
// ============================================================
const WAQI_TOKEN = '4e8a7681495d03130e89e15bf00c32368b90a133'; // 🔴 replace this
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  showLoading(false);
  requestLocationPermission(); // 📍 ask for location immediately on load
});

// ============================================================
// 📍 LOCATION — ask on load, show banner if denied
// ============================================================
function requestLocationPermission() {
  if (!navigator.geolocation) {
    showLocationBanner('Your browser does not support location. Please type your city manually.');
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (err) => {
      // ❌ Denied — show red banner forcing them to enable
      showLocationBanner('📍 OxyTrace needs your location for accurate AQI data. Please enable location permission and click Try Again.');
    },
...
function showLocationBanner(msg) {
  let banner = document.getElementById('locationBanner');
  if (!banner) {
    banner = document.createElement('div');
    banner.id = 'locationBanner';
    banner.style.cssText = `
      position: fixed; top: 0; left: 0; right: 0; z-index: 9999;
      background: #f44336; color: white; text-align: center;
  ...
  banner.innerHTML = `
    ...
    <button onclick="requestLocationPermission()" style="
      background: white; color: #f44336; border: none; border-radius: 6px;
      padding: 6px 16px; font-weight: 700; cursor: pointer; font-size: 14px;
    ">Try Again</button>
  `;
}
...
async function prefillCityFromCoords(lat, lon) {
  try {
    ...
    if (data.status === 'ok') {
      ...
      if (cityInput) cityInput.value = data.data.city.name;
    }
  } catch (e) {
    console.warn('Could not prefill city:', e);
  }
}
...
async function startMonitoring() {
  ...
  try {
    if (userCoords) {
      await fetchAQIByCoords(userCoords.lat, userCoords.lon);
    } else {
      await fetchAQIByCity(city);
    }
  } catch (e) {
  ...
  // Fire and forget — don't block dashboard if Firebase fails
  saveUserSession({
...
// ============================================================
// AQI FETCH — by GPS coords (most accurate)
// ============================================================
...
// ============================================================
// AQI FETCH — by city name (fallback)
// ============================================================
...
function renderAlerts(riskData) {
  ...
  if (hour >= 7 && hour <= 10)       alerts.push({ type: 'warning', icon: '🚗', msg: 'Morning rush hour: Traffic emissions at peak. Avoid roadside exposure.' });
  else if (hour >= 18 && hour <= 21) alerts.push({ type: 'warning', icon: '🌆', msg: 'Evening pollution spike detected. Consider indoor exercise today.' });
...
function renderTimeline() {
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    history.push({ time: t.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), aqi, isNow: i === 0 });
  }
...
function renderPreventionTips(riskData) {
  ...
  if (userProfile.exercise < 2) tips.push({ icon: '🏃', title: 'Exercise Indoors',        desc: 'Regular cardio strengthens respiratory muscles and lung capacity.' });
  ...
  if (userProfile.workEnv > 0.5) tips.push({ icon: '🦺', title: 'Use Respirator at Work', desc: 'Industrial dust causes irreversible lung scarring over time.' });
  if (userProfile.age > 50)      tips.push({ icon: '🩺', title: 'Annual Lung Screening',  desc: 'CT scan screening is recommended annually for your age group.' });
...
async function refreshAQI() {
  ...
  try {
    if (userCoords) {
      ...
      await fetchAQIByCity(userProfile.city);
    }
  } catch (e) {
    simulateAQIData();
  }
