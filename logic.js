...
const VAPID_KEY = 'PASTE_YOUR_VAPID_KEY_HERE'; // ← from Step 1

let _messaging = null;
let _notifLastSent = 0;
const NOTIF_COOLDOWN_MS = 10 * 60 * 1000; // max 1 notification per 10 mins
...
function sendNotification(title, body) {
  ...
  if (Notification.permission === 'granted') {
    new Notification(title, {
      ...
      vibrate: [200, 100, 200],
      tag: 'oxytrace-alert', // replaces previous notification instead of stacking
      renotify: true
    });
  }
}
...
const checkAlertStatus = (aqi, riskLevel, message) => {
  ...
  const title = `OxyTrace: ${riskLevel} Air Quality Alert`;
...
// ─── BOOT ─────────────────────────────────────────────────────────────────────
// Ask for notification permission as soon as page loads
